...
function opUpdate($sn){
  global $db,$ugmKind,$WEB;
  if(!$sn) redirect_header($_SESSION['returnUrl'], 3000, "癌症項目名稱錯誤！");
  
  #驗證token
  verifyToken($_POST['token']);

  #過濾
  $_POST['title'] = db_CleanVars($_POST['title'], "癌症項目名稱");
  ...
  #更新資料庫
  $sql = "update `{$WEB['moduleName']}_cancer` set 
          `title` = '{$_POST['title']}',
          `kind` = '{$_POST['kind']}',
          `enable` = '{$_POST['enable']}',
          `date` = '{$_POST['date']}',
...
function opInsert(){
  ...
  #寫進資料庫
  $sql = "insert into `{$WEB['moduleName']}_cancer`
        (`title`,`kind`,`enable`,`date`,`summary`,`content`) values
        ('{$_POST['title']}','{$_POST['kind']}','{$_POST['enable']}','{$_POST['date']}','{$_POST['summary']}','{$_POST['content']}')"; 
  //die($sql);
...
function opList(){
  ...
  $thead = array(
    ...
    "title" => array("content" => "癌症項目名稱","attr"=>["class" => "text-center col-sm-2"]),
    "summary" => array("content" => "摘要","attr"=>["class" => "text-center col-sm-3"]),
  ...
  $tbody = array(
    ...
    "summary" => array("attr"=>["class" => "text-left"]),

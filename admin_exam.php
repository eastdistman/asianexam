...
function opUpdate($sn){
  global $db,$ugmKind,$WEB;
  if(!$sn) redirect_header($_SESSION['returnUrl'], 3000, "健檢項目名稱錯誤！");
  
  #驗證token
  verifyToken($_POST['token']);

  #過濾
  $_POST['title'] = db_CleanVars($_POST['title'], "健檢項目名稱");
  ...
  #更新資料庫
  $sql = "update `{$WEB['moduleName']}_exam` set 
          `title` = '{$_POST['title']}',
          `kind` = '{$_POST['kind']}',
          `enable` = '{$_POST['enable']}',
          `date` = '{$_POST['date']}',
...
function opInsert(){
  ...
  #寫進資料庫
  $sql = "insert into `{$WEB['moduleName']}_exam`
...
function opList(){
  ...
  $thead = array(
    ...
    "title" => array("content" => "健檢項目名稱","attr"=>["class" => "text-center col-sm-2"]),

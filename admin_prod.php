...
function opUpdate($sn){
  global $db,$ugmKind,$WEB;
  if(!$sn) redirect_header($_SESSION['returnUrl'], 3000, "檢查項目名稱錯誤！");
  
  #驗證token
  verifyToken($_POST['token']);

  #過濾
  $_POST['title'] = db_CleanVars($_POST['title'], "檢查項目名稱");
...
function opList(){
  ...
  $thead = array(
    ...
    "title" => array("content" => "檢驗項目名稱","attr"=>["class" => "text-center col-sm-2"]),
